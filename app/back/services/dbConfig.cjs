...
const TABLE_NAMES = {
  ...
  remisiones: 'ven_remisiones',
  remisiones_detalle: 'ven_detaremision',
...
const QUERIES = {
  ...
  GET_COTIZACIONES: `
    SELECT 
      ...
      LTRIM(RTRIM(c.codter)) AS codter,
  ...
  GET_COTIZACIONES_DETALLE: `
    SELECT 
      ...
      COALESCE(p_resolved.codins, LTRIM(RTRIM(d.coddet))) AS codProducto,
      ...
      (d.candet * d.vundet) * (COALESCE(d.dctdet, 0) / 100.0) AS descuentoValor,
      -- Descripción
      COALESCE(p_resolved.nomins, LTRIM(RTRIM(d.nomdet)), '') AS descripcion
    FROM ${TABLE_NAMES.cotizaciones_detalle} d
    OUTER APPLY (
        SELECT TOP 1 id, codins, nomins 
        FROM ${TABLE_NAMES.productos} i 
        WHERE LTRIM(RTRIM(i.codins)) = LTRIM(RTRIM(d.coddet))
    ) p_resolved
  `,
  ...
  GET_PEDIDOS: `
    SELECT 
      p.id,
      LTRIM(RTRIM(p.numped)) as numeroPedido,
      p.fecped as fechaPedido,
      LTRIM(RTRIM(COALESCE(p.codcli, ''))) as clienteId,
      CASE 
        WHEN cli.nomter IS NOT NULL AND LTRIM(RTRIM(cli.nomter)) != '' 
        THEN LTRIM(RTRIM(cli.nomter))
        ELSE NULL
      ...
      LTRIM(RTRIM(p.numcot)) as cotizacionId,
      ...
      p.estped as estado,
      p.codalm as empresaId,
      NULL as fechaEntregaEstimada,
      p.codtar as listaPrecioId,
      0 as descuentoPorcentaje,
      0 as ivaPorcentaje,
      0 as impoconsumoValor,
      NULL as instruccionesEntrega
    FROM ${TABLE_NAMES.pedidos} p
    LEFT JOIN ${TABLE_NAMES.clientes} cli ON RTRIM(LTRIM(cli.codter)) = RTRIM(LTRIM(p.codcli))
    ORDER BY p.fecped DESC
  `,
  ...
  GET_PEDIDOS_DETALLE: `
    SELECT 
      ...
      pd.numped,
      NULL as pedidoId,
      -- Obtener el ID del producto desde inv_insumos (código normal)
      COALESCE(
      ...
      -- Calcular ivaPorcentaje
      CASE 
        WHEN COALESCE(pd.canped, 0) > 0 AND COALESCE(pd.valins, 0) > 0 
          AND (COALESCE(pd.canped, 0) * COALESCE(pd.valins, 0) - COALESCE(pd.dctped, 0)) > 0
        THEN (COALESCE(pd.ivaped, 0) / (COALESCE(pd.canped, 0) * COALESCE(pd.valins, 0) - COALESCE(pd.dctped, 0))) * 100
        ELSE 0
  ...
  GET_REMISIONES: `
    SELECT 
      ...
      (
        ...
        -- REMOVED BROKEN JOIN: LEFT JOIN ${TABLE_NAMES.pedidos} ped ON ped.id = r.pedido_id
        -- REMOVED BROKEN JOIN: LEFT JOIN ${TABLE_NAMES.pedidos_detalle} pd
  ...
  GET_REMISIONES_DETALLE: `
    SELECT 
      ...
      -- 1. PRECIO UNITARIO: Fallback al catálogo
         COALESCE(p.ultimo_costo, 0) as precioUnitario,

      -- 2. DESCUENTO: 0 por ahora (JOIN roto)
      0 as descuentoPorcentaje,
      ...
      -- 4. CAMPOS CALCULADOS
      -- Subtotal
      (rd.cantidad_enviada * COALESCE(p.ultimo_costo, 0)) as subtotal,

      -- Valor IVA
      0 as valorIva,

      -- Total
      (rd.cantidad_enviada * COALESCE(p.ultimo_costo, 0)) as total
