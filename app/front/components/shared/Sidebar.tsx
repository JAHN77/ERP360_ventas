...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 bg-white/30 dark:bg-black/30 backdrop-blur-md z-20 transition-all duration-300 md:hidden ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setIsSidebarOpen(false)}
      ></div>

      <div className={`fixed inset-y-0 left-0 flex flex-col w-64 bg-white dark:bg-slate-800 shadow-xl z-30 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex-shrink-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}>
