  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      const dataToSubmit: Omit<Cliente, 'id' | 'condicionPago' | 'activo' | 'createdAt' | 'nombreCompleto'> = {
        ...
        tipter: isPersonaJuridica ? 2 : 1,
      };
      onSubmit(dataToSubmit);
    }
  };

  const getInputClasses = (fieldName: keyof typeof formData) => `mt-1 block w-full px-3 py-2 bg-white dark:bg-slate-700 border rounded-md focus:outline-none focus:ring-2 dark:text-slate-200 ${
