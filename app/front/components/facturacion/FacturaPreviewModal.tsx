...
const FacturaPreviewModal: React.FC<FacturaPreviewModalProps> = ({ factura, onClose, onTimbrar }) => {
    ...
    const { clientes, datosEmpresa, vendedores, productos } = useData();
    ...
    useEffect(() => {
        const fetchPdfPreview = async () => {
            ...
            try {
                // Generar PDF localmente usando FacturaPDFDocument
                const blob = await pdf(
                    <FacturaPDFDocument
                        ...
                        productos={productos}
                        firmaVendedor={null} // Opcional, si se tiene la firma
                    />
                ).toBlob();

                const url = URL.createObjectURL(blob);
                setPdfPreviewUrl(url);
                setPdfFilename(`Factura-${factura.numeroFactura}.pdf`);

            } catch (error: any) {
                console.error("Error generating local PDF preview:", error);
                addNotification({ message: "Error al generar la vista previa del PDF.", type: "error" });
            } finally {
        ...
        // Cleanup function to revoke ObjectURL
        return () => {
            // Not easy to revoke here if we want to keep it while modal is open, 
            // but strictly we should revoke when pdfPreviewUrl changes or component unmounts.
            // For now we rely on garbage collection or simple overwrite.
        };

    }, [factura, cliente, datosEmpresa, preferences, productos, vendedor]); // dependencies updated
    ...
    const handleConfirmSendEmail = async (emailData: { to: string; subject: string; body: string }) => {
        ...
        setIsGenerating(true);
        addNotification({ message: 'Enviando correo con PDF...', type: 'info' });

        try {
            // Obtener blob directo de la URL generada localmente
